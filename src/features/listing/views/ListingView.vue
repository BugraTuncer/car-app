...
const sortDirection = computed<SortDirection | null>(() =>
  route.query.sortDirection !== undefined ? Number(route.query.sortDirection) : null
)
...
function buildListingParams(overrides: Partial<ListingParams> = {}): ListingParams {
  ...
  if (query.sortDirection !== undefined) params.sortDirection = Number(query.sortDirection)
...
function updateURL(newParams: LocationQueryRaw) {
  router.push({
    query: {
      ...route.query,
      ...newParams
...
<template>
  <div>
    ...
    <div v-else-if="items.length === 0" class="listing-status">Sonuç bulunamadı.</div>
    ...
    <div v-if="isMobile && items.length > 0" ref="scrollSentinel" class="infinite-scroll-sentinel">
      <div v-if="loadingMore" class="infinite-scroll-loading">Yükleniyor...</div>
      <div v-else-if="!hasMore" class="infinite-scroll-end">Tüm sonuçlar yüklendi</div>
    </div>
